...
namespace BCA007.Controllers.Attendance
{
    ...
    {
        private readonly IPeriodService _serv;

        public PeriodController(IPeriodService serv)
        {
            _serv = serv;
        }
        [HttpGet("getall")]
        ...
        [HttpPost("create")]
        public async Task<ActionResult<PeriodDto>> Create(PeriodDto dto)
        {
        ...
        [HttpPut("edit")]
        public async Task<ActionResult<PeriodDto>> Update(PeriodDto dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            try
