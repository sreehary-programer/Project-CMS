...
namespace BCA007.Controllers.Library
{
    ...
    {
        private readonly IBookTypeService _serv;

        public BookTypeController(IBookTypeService serv)
        {
            _serv = serv;
        }
        [HttpGet("getall")]
        ...
        [HttpPost("create")]
        public async Task<ActionResult<BookTypeDto>> Create(BookTypeDto dto)
        {
        ...
        [HttpPut("edit")]
        public async Task<ActionResult<BookTypeDto>> Update(BookTypeDto dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            try
