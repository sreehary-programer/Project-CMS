...
namespace BCA007.Controllers.Library
{
    ...
    {
        private readonly IBookPublisherService _serv;

        public BookPublisherController(IBookPublisherService serv)
        {
            _serv = serv;
        }
        [HttpGet("getall")]
        ...
        [HttpPost("create")]
        public async Task<ActionResult<BookPublisherDto>> Create(BookPublisherDto dto)
        {
        ...
        [HttpPut("edit")]
        public async Task<ActionResult<BookPublisherDto>> Update(BookPublisherDto dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            try
