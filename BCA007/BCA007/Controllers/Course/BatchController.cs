...
namespace BCA007.Controllers.Course
{
    ...
    {
        private readonly IBatchService _serv;

        public BatchController(IBatchService serv)
        {
            _serv = serv;
        }
        [HttpGet("getall")]
        ...
        [HttpPost("create")]
        public async Task<ActionResult<BatchDto>> Create(BatchDto dto)
        {
        ...
        [HttpPut("edit")]
        public async Task<ActionResult<BatchDto>> Update(BatchDto dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            try
