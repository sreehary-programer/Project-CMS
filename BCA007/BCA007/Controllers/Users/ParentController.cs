...
namespace BCA007.Controllers.Users
{
    ...
    public class ParentController : ControllerBase
    {
        ...
        [HttpPost("create")]
        public async Task<ActionResult<ParentDto>> Create([FromForm] string dto, IFormFile? file)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            try
            {
                var model = JsonSerializer.Deserialize<ParentDto>(dto)!;
        ...
        [HttpPut("edit")]
        public async Task<ActionResult<ParentDto>> Update([FromForm] string dto, IFormFile? file)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            try
