...
namespace BCA007.Controllers.Bus
{
    ...
    {
        private readonly IBusAssignmentService _serv;

        public BusAssignmentController(IBusAssignmentService serv)
        {
            _serv = serv;
        }
        [HttpGet("getall")]
        ...
        [HttpPost("create")]
        public async Task<ActionResult<BusAssignmentDto>> Create(BusAssignmentDto dto)
        {
        ...
        [HttpPut("edit")]
        public async Task<ActionResult<BusAssignmentDto>> Update(BusAssignmentDto dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            try
